                    ...
                    {userLessons.map(lesson => (
                        <Card key={lesson.id} className="p-4 border-white/5 bg-slate-900/40 flex items-center justify-between group">
                            ...
                            <div className="flex gap-2">
                                <Button variant="ghost" size="icon" onClick={() => deleteLesson(lesson.id)} aria-label={`Delete lesson ${lesson.title}`}>
                                    <Trash2 size={16} className="text-slate-600 hover:text-red-400" />
                                </Button>
                                <Button variant="secondary" size="icon" aria-label={`Edit lesson ${lesson.title}`} onClick={() => {
                                    setCurrentLesson(lesson);
                                    setStep(lesson.type === 'deck' ? 'create_deck' : 'create_quiz');
                                }}>
                                    <Edit3 size={16} />
                                </Button>
                                <Button variant="primary" size="icon" onClick={() => startStudy(lesson)} aria-label={`Start lesson ${lesson.title}`}>
