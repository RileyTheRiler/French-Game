...
// Fallback items if market utility fails or for standard supplies
...
const ShopModal = ({ onClose }) => {
    ...
    useEffect(() => {
        ...
        // Try to get dynamic selection, fallback to standard supplies if needed
        let selection;
        if (getDailyShopSelection) {
             selection = getDailyShopSelection(today);
        }

        // Ensure we have at least standard supplies
        const consumables = selection?.consumables?.length > 0 ? selection.consumables : STANDARD_SUPPLIES;
        const cosmetics = selection?.cosmetics || [];
    ...
    const handleBuy = (item) => {
        ...
        if (success) {
            // Activate Double XP immediately upon purchase if it's a potion
            if (item.id === 'double_xp') {
                if (activateDoubleXP) activateDoubleXP(15);
            } else if (item.id === 'xp_boost_30') {
                if (activateDoubleXP) activateDoubleXP(30);
            }
