...
const ShopModal = ({ onClose }) => {
    ...
    const handleBuy = (item) => {
        ...
        if (success) {
            ...
            if (item.effect?.type === 'double_xp' || item.id === 'double_xp') {
                activateDoubleXP(15);
            } else if (item.id === 'xp_boost_30') {
                activateDoubleXP(30);
            }
            // other effects like 'add_streak_freeze' are handled by buyItem updating inventory
    ...
    const renderItemCard = (item, isFeatured = false) => {
        ...
        // Dynamic icon rendering if icon is a string (emoji) or element
        const renderIcon = () => {
            if (typeof item.icon === 'string') return item.icon;
            return item.icon; // React element
        };
