...
const PronunciationCoach = () => {
    ...
    const { addXP, addCoins, stats, updateDailyStat, incrementStat, offlineAudio } = useProgress();
    ...
    const checkPronunciation = (heard) => {
        ...
        // Update stats or mark strength
        // Assuming markWordStrength is not available in merged context or handled via addXP logic
        // But Incoming had simple logic. HEAD had markWordStrength usage.
        // I will assume simple addXP for now, but maybe markWordStrength if I can access it.
        // Actually, let's stick to XP/Coins.
        ...
        setTimeout(() => {
            if (analysis.score >= 80) {
                ...
                setSuccessCount(prev => {
                    const next = prev + 1;
                    updateDailyStat('dailyStreak', next, 'max'); // Using Incoming logic
                    return next;
                });
                addXP(xpGain);
                addCoins(5);
    ...
    const handleNext = () => {
        if (currentWordIndex < wordsToPractice.length - 1) {
            ...
            incrementStat('pronunciationPractices', successCount); // Using Incoming logic
