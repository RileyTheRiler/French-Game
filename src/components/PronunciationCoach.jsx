...
const PronunciationCoach = () => {
    ...
    const { addXP, addCoins, updateDailyStat, incrementStat, markWordStrength, offlineAudio, stats } = useProgress();
    ...
    const checkPronunciation = (heard) => {
        ...
        // Use the advanced PronunciationAnalyzer if available
        let analysis;
        if (analyzePronunciation) {
            analysis = analyzePronunciation(currentWord, heard);
        } else {
            // Fallback
            const { accuracy } = scorePronunciation(currentWord.french, heard);
            analysis = { score: accuracy, feedback: {} };
        }
    ...
    const handleNext = () => {
        if (currentWordIndex < wordsToPractice.length - 1) {
            ...
            // XP/Coins already added per word?
            // The other branch added coins per word (5).
            // But usually session reward is a bonus.
            // Let's add the session reward bonus.
