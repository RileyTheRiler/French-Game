...
const PronunciationCoach = () => {
        ...
        setTimeout(() => {
            if (analysis.score >= 80) {
                ...
                setSuccessCount(prev => {
                    const next = prev + 1;
                    updateDailyStat('dailyStreak', next, 'max'); // Update streak if success
                    return next;
                });
                addXP(xpGain);
                if (analysis.score >= 90) addCoins(5); // Bonus coins for good pronunciation
            } else {
                setStatus('fail');
    ...
    const handleNext = () => {
        if (currentWordIndex < wordsToPractice.length - 1) {
            ...
            // Rewards already added per word? No, reward system usually calculates session bonus.
            // But we added XP per word. Let's add session completion bonus.
            addXP(reward.xp);
            addCoins(reward.coins);
            incrementStat('pronunciationPractices', 1);
