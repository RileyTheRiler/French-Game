                ...
                >
                    <Card className="p-0 border-white/10 shadow-3xl overflow-hidden">
                        ...
                        <div className="p-6 border-t border-white/10 flex justify-between items-center">
                            ...
                            <div className="flex gap-1" role="group" aria-label="Grammar tip navigation">
                                {GRAMMAR_TIPS.map((_, idx) => (
                                    <button
                                        key={idx}
                                        onClick={() => setCurrentIndex(idx)}
                                        aria-label={`Go to grammar tip ${idx + 1}`}
                                        aria-current={idx === currentIndex ? "true" : undefined}
                                        className={`w-2 h-2 rounded-full transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-400 focus-visible:ring-offset-2 focus-visible:ring-offset-slate-900 ${idx === currentIndex ? 'bg-emerald-500 w-6' : 'bg-slate-700 hover:bg-slate-600'
                                            }`}
                                    />
                                ))}
                            </div>
