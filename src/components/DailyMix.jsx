        ...
        >
            <div id="daily-mix-container" className="max-w-2xl mx-auto w-full flex-1 flex flex-col justify-center p-4">
                {currentChallenge.type === CHALLENGE_TYPES.GRAMMAR_CHECK && (
                    <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
                        <div className="mb-8 text-center">
                            <Badge className="mb-4 bg-purple-500/20 text-purple-300 border-purple-500/30">Grammar Drill</Badge>
                ...
                {currentChallenge.type === CHALLENGE_TYPES.CONTEXT_CLOZE && currentChallenge.clozeData && (
                    <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
                        <div className="mb-6 text-center">
                            <Badge className="bg-purple-500/20 text-purple-300 border-purple-500/30">Context Cloze</Badge>
                            <h2 className="text-2xl md:text-3xl font-bold text-white mb-2 leading-relaxed">
                                {currentChallenge.clozeData.question.split('_____').map((part, i, arr) => (
                                    <React.Fragment key={i}>
                                        {part}
                                        {i < arr.length - 1 && (
                                            <span className={`inline-block px-3 py-1 mx-1 rounded-lg border-b-2 min-w-[80px] ${isAnswered && isCorrect ? 'bg-emerald-500/20 border-emerald-500 text-emerald-300' : ''} ${isAnswered && !isCorrect ? 'bg-red-500/20 border-red-500 text-red-300' : ''} ${!isAnswered ? 'bg-white/10 border-white/30 text-transparent' : ''}`}>
                    ...
                    <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
                        <div className="mb-12 text-center flex flex-col items-center">
                            {currentChallenge.type === CHALLENGE_TYPES.LISTENING ? (
                                <Button
                                    className="w-32 h-32 rounded-full bg-indigo-600 shadow-xl hover:scale-105 transition-transform"
                ...
                {currentChallenge.type === CHALLENGE_TYPES.REVERSE_FLASHCARD && (
                    <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
                        <div className="mb-6 text-center">
                            <Badge variant="outline" className="mx-auto mb-4 border-amber-500/30 text-amber-300">Memory Recall</Badge>
                        ...
                        {!isRevealed ? (
                            ...
                            <div className="w-full bg-indigo-900/20 border border-indigo-500/30 rounded-3xl p-8 text-center animate-in zoom-in-95 duration-200">
                                ...
                                <div className="flex gap-3 mt-8">
                                    <Button className="flex-1 bg-red-500/20 text-red-400 hover:bg-red-500/30 border-red-500/20" onClick={() => handleFlashcardGrade('again')}>Forgot</Button>
                                    <Button className="flex-1 bg-emerald-500/20 text-emerald-400 hover:bg-emerald-500/30 border-emerald-500/20" onClick={() => handleFlashcardGrade('good')}>Got it</Button>
                                </div>
                            </div>
                        )}
                    </div>
