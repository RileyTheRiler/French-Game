...
const FallingWordsGame = () => {
    ...
    const { addXP, updateDailyStat, logWordAttempt, globalDifficulty, difficultySettings } = useProgress();
    ...
    const gameLoop = (time) => {
        ...
        if (!isZenModeRef.current && !gameOver) {
            ...
            if (timeLeftRef.current <= 0) {
                ...
                setTimeLeft(0);
                // Need to access current score synchronously?
                // We can't access state easily here without ref or passing it.
                // But recordingRef has the latest score?
                const latest = recordingRef.current[recordingRef.current.length - 1]?.score || 0;
                endGame(latest);
                return; // Stop loop
            }
