...
const FallingWordsGame = () => {
    ...
    const { getDueWords, updateWordProgress, CATEGORIES, getWeightedPracticeWords, vocabulary, markWordSeen, getPracticeQueue } = useVocabulary();
    const { stats, recordCategoryPerformance, setModeDifficulty, addXP, addCoins, updateDailyStat, incrementStat, logWordAttempt, globalDifficulty, difficultySettings, offlineAudio } = useProgress();
    ...
    useEffect(() => {
        ...
        try {
            const words = getPracticeQueue ? getPracticeQueue('fallingWords', 40) : getDueWords();
            const weighted = getWeightedPracticeWords ? getWeightedPracticeWords(40) : getDueWords();
            const finalWords = (words && words.length) ? words : ((weighted && weighted.length) ? weighted : getDueWords());

            if (!finalWords || finalWords.length === 0) {
                console.warn("No words available!");
                validWords.current = vocabulary || [];
            } else {
                validWords.current = finalWords;
            }
        } catch (e) {
    ...
    const spawnWord = () => {
        ...
        if (markWordSeen) markWordSeen(randomWord.id);
    ...
    const grantSessionRewards = useCallback(() => {
        ...
        // Assume 3 lives if not Zen mode, but we don't track lives explicitly in Time Attack
        // But let's pass dummy lives to calculation
        const lives = isZenModeRef.current ? 3 : 1;
    ...
    const gameLoop = (time) => {
        ...
        activeWordsRef.current.forEach(word => {
            ...
            if (newY > 100) {
                ...
                logWordAttempt(word.category || 'General', false, performance.now() - word.spawnTime, word.wordId);
    ...
    const endGame = (finalScore) => {
        ...
        // Grant rewards as per other branch logic (using accumulated stats)
        // Since we don't have explicit grantSessionRewards call inside loop, we call it here via effect or direct
        // Effect handles it: useEffect(() => { if (gameOver) grantSessionRewards(); }, [gameOver]);
    };
    ...
    const handleInputChange = (e) => {
        ...
        if (matchIndex !== -1) {
            ...
            logWordAttempt(word.category || 'General', true, performance.now() - word.spawnTime, word.wordId);
            ...
            setScore(s => {
                const newScore = Math.floor(s + (10 * comboMultiplier * accuracyBoost * speedBoost));
                // Record event
        ...
        <GameLayout
            ...
            headerRight={
                <div className="flex items-center gap-4">
                    {/* Difficulty Slider from Other Branch */}
                    {!isRivalsMode && (
                        <div className="hidden md:block w-32 lg:w-52">
                            <DifficultySlider
                                value={difficulty}
                        ...
                        <WordItem
                            ...
                            hint={
                                difficultySettings?.learnerType === 'scholar'
                                    ? `[${word.target.gender || word.category || '?'}]`
