...
const FallingWordsGame = () => {
    ...
    const performanceSummaryRef = useRef({ averageAccuracy: 1, averageResponse: 2000, lowestCategory: null });
    ...
    useEffect(() => {
        ...
        try {
            ...
            const finalList = (words && words.length) ? words : ((weighted && weighted.length) ? weighted : getDueWords());

            if (!finalList || finalList.length === 0) {
                console.warn("No words available!");
                validWords.current = vocabulary || [];
            } else {
                validWords.current = finalList;
            }
        } catch (e) {
    ...
    const getCategoryAccuracy = (category) => {
        if (!stats?.categoryPerformance?.[category]) return 0.85;
    ...
    const grantSessionRewards = useCallback(() => {
        ...
        const reward = calculateRewards ? calculateRewards('fallingWords', {
            ...
            maxCombo,
            wordsCaught,
            livesRemaining: 3, // HEAD mode has no lives, assuming max
    ...
    const gameLoop = (time) => {
        ...
        activeWordsRef.current.forEach(word => {
            ...
            const categoryPressure = 1 + ((1 - (word.categoryAccuracy || 0.8)) * 0.3);
    ...
    const hintData = useMemo(() => {
        const strugglingCategory = performanceSummaryRef.current.lowestCategory?.category;
        if (!strugglingCategory) return null;

        const word = renderedWords.find(w => w.category === strugglingCategory);
        if (!word) return null;

        const allowHint = (performanceSummaryRef.current.lowestCategory?.accuracy || 1) < 0.78 || difficulty <= 2;
