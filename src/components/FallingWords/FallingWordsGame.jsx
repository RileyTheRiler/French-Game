...
const FallingWordsGame = () => {
    ...
    const { getDueWords, updateWordProgress, CATEGORIES, getWeightedPracticeWords, vocabulary, getPracticeQueue, markWordSeen } = useVocabulary();
    const { stats, recordCategoryPerformance, setModeDifficulty, addXP, addCoins, updateDailyStat, incrementStat, offlineAudio, globalDifficulty, difficultySettings } = useProgress();
    ...
    const timeLeftRef = useRef(INITIAL_TIME_SECONDS);
    ...
    useEffect(() => {
        ...
        recognitionRef.current.onresult = (event) => {
            ...
            if (target) {
                const { accuracy } = scorePronunciation(target.text, heard);
    ...
    const getCategoryResponse = useCallback((category) => {
        const perf = categoryPerformance[category];
        return perf?.averageResponseTime || 2000;
    }, [categoryPerformance]);

    // Initialize Game Logic
    useEffect(() => {
        const savedGhost = localStorage.getItem(GHOST_STORAGE_KEY);
        if (savedGhost) setHasGhostData(true);

        try {
            const words = getPracticeQueue('fallingWords', 40);
            const weighted = getWeightedPracticeWords ? getWeightedPracticeWords(40) : [];
            const valid = (words && words.length) ? words : (weighted.length ? weighted : getDueWords());

            if (!valid || valid.length === 0) {
                validWords.current = vocabulary || [];
            } else {
                validWords.current = valid;
            }
        } catch (e) {
    ...
    const spawnWord = () => {
        ...
        lastHeardWordRef.current = newWord;
    ...
    const gameLoop = (time) => {
        ...
        if (isGhostModeRef.current && ghostDataRef.current.length > 0) {
            ...
            if (latestEvent) setGhostScore(latestEvent.score);
        }
        ...
        // Difficulty & Spawning
        let difficultyProgress = isZenModeRef.current ? 0 : Math.min(timeElapsed / TIME_TO_MAX_DIFFICULTY, 1.0);
        const flowMultiplier = 1 + (combo * 0.05);
        const band = bandRef.current;
        const difficultyMultiplier = (globalDifficulty / 50) || 1.0;

        const effectiveSpeed = currentFallSpeedRef.current * flowMultiplier * difficultyMultiplier * (band?.speed || 1);
        ...
        currentSpawnIntervalRef.current = Math.max(
            (baseInterval / difficultyMultiplier) / (1 + (combo * 0.1)),
    ...
    const grantSessionRewards = useCallback(() => {
        ...
        const reward = calculateRewards('fallingWords', {
            ...
            maxCombo,
            wordsCaught,
            livesRemaining: 3, // Mock lives since we use timer
    ...
    const handleInputChange = (e) => {
        ...
        if (matchIndex !== -1) {
            ...
            recordCategoryPerformance(word.category, {
                success: true,
                responseTime: performance.now() - word.spawnTime,
                mode: 'fallingWords'
            });
            ...
            setScore(s => {
                const newScore = Math.floor(s + (12 * comboMultiplier * bandScore));
    ...
    useEffect(() => {
        if (inputRef.current && !gameOver) inputRef.current.focus();
    });

    // Hints
    const hintData = useMemo(() => {
        // Simple hint logic for now
        const word = renderedWords[0];
        if (!word) return null;
        if (difficulty > 2) return null; // No hints on hard

        const masked = `${word.text[0]}${'•'.repeat(Math.max(0, word.text.length - 1))}`;
        return {
            masked,
            translation: word.translation,
            name: CATEGORIES?.[word.category]?.name || word.category
        };
    }, [renderedWords, difficulty, CATEGORIES]);
                ...
                <div className="w-full h-full relative">
                    {renderedWords.map(word => (
                        <WordItem
                            ...
                            y={word.y}
                            isMatched={false}
                            hint={difficultyConfig.showInitial ? word.text.charAt(0) : null}
                    ...
                    <div className="fixed inset-0 bg-slate-950/90 backdrop-blur-md flex items-center justify-center z-50">
                        ...
                        >
                            <Card className="text-center p-12 border-white/10 shadow-3xl">
                                ...
                                <p className="text-3xl font-bold mb-8">Score: {score}</p>
                                {sessionReward && (
                                    <div className="flex justify-around mb-6">
                                        <div><p>XP</p><p className="font-bold">+{sessionReward.xp}</p></div>
                                        <div><p>Coins</p><p className="font-bold">+{sessionReward.coins}</p></div>
                                    </div>
                                )}
                                <div className="flex flex-col gap-3">
                                    <Button size="lg" onClick={restartGame}>Play Again</Button>
                                    <Button variant="ghost" onClick={onExit}>Menu</Button>
                                </div>
                            </Card>
                        </motion.div>
                    </div>
