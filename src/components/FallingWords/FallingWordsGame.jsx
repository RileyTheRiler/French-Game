...
const FallingWordsGame = () => {
    ...
    const { getDueWords, updateWordProgress, getPracticeQueue, markWordSeen, CATEGORIES, vocabulary } = useVocabulary();
    const {
        stats,
    ...
    useEffect(() => {
        ...
        try {
            ...
            if (!words || words.length === 0) {
                // Fallback if practice queue is empty
                console.warn("No words available in practice queue, falling back to due words");
                const dueWords = getDueWords();
                if (dueWords && dueWords.length > 0) {
                    validWords.current = dueWords;
                } else {
                     console.warn("No due words available!");
    ...
    }, [getPracticeQueue, getDueWords, vocabulary]);
    ...
    const gameLoop = (time) => {
        ...
        // Combine logic from both branches for speed calculation
        ...
        activeWordsRef.current.forEach(word => {
            ...
            if (newY > 100) {
                mistakes++;
                if (!isZenModeRef.current) setLives(l => Math.max(0, l - 1));
