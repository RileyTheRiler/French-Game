...
const FallingWordsGame = () => {
    ...
    const { getDueWords, updateWordProgress, getWeightedPracticeWords, vocabulary, getPracticeQueue, markWordSeen, CATEGORIES } = useVocabulary();
    const { stats, recordCategoryPerformance, setModeDifficulty, addXP, addCoins, updateDailyStat, incrementStat, logWordAttempt, globalDifficulty, offlineAudio } = useProgress();
    ...
    useEffect(() => {
        ...
        try {
            ...
            const wordsToUse = weighted && weighted.length ? weighted : (words && words.length ? words : getDueWords());
    ...
    }, []); // Removed deps to avoid restart loop, run once on mount
    ...
    const gameLoop = (time) => {
        ...
        activeWordsRef.current.forEach(word => {
            ...
            const responseRelief = (word.target?.priorityScore || 0) > 2 ? 0.85 : 1;
    ...
    const hintData = useMemo(() => {
        ...
        const allowHint = (performanceSummary.lowestCategory?.accuracy || 1) < 0.78 || difficultySetting <= 2;
