...
const FallingWordsGame = () => {
    ...
    const { stats, recordCategoryPerformance, setModeDifficulty, addXP, addCoins, updateDailyStat, incrementStat, logWordAttempt, globalDifficulty, difficultySettings } = useProgress();
    const { offlineAudio } = useProgress();
    const { getPracticeQueue, markWordSeen } = useVocabulary();
    ...
    const grantSessionRewards = useCallback(() => {
        ...
        // Simple reward logic if util is missing
        const reward = { xp: score, coins: Math.floor(score / 10) };
    ...
    const startShadowing = () => {
        ...
        // Shadowing implementation placeholder
    };
