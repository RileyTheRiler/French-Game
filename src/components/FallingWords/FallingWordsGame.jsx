...
const INITIAL_LIVES = 3; // Kept if needed, but Time Attack uses time
...
const FallingWordsGame = () => {
    ...
    useEffect(() => {
        ...
        try {
            // Resolve conflict: use getPracticeQueue or getDueWords logic
            // Memory says tests depend on getPracticeQueue OR getDueWords being mocked.
            // I'll try to use a robust fallback chain.
            let words = [];
            if (getPracticeQueue) {
                 words = getPracticeQueue('fallingWords', 40);
            }
            if (!words || words.length === 0) {
                 words = getWeightedPracticeWords ? getWeightedPracticeWords(40) : [];
            }
            if (!words || words.length === 0) {
                 words = getDueWords();
            }
    ...
    }, [getPracticeQueue, getDueWords, getWeightedPracticeWords, vocabulary]); // Added deps
    ...
    const spawnWord = () => {
        ...
        const newWord = {
            ...
            y: -10,
            isMatched: false,
            spawnTime: now, // Used by WordItem (hint) and logic
            spawnedAt: now, // Used by logic (duplicated, but keeping for compatibility if needed)
    ...
    const grantSessionRewards = useCallback(() => {
        ...
        // Use time attack / lives logic?
        // Incoming used `livesRemaining: lives`. But we don't have lives state.
        // We use Time Attack mode where game over is time up.
        // So livesRemaining is effectively 0 or N/A.
        const reward = calculateRewards('fallingWords', {
            score,
            maxCombo,
            wordsCaught,
            livesRemaining: 3, // Dummy value
    ...
    const hintData = useMemo(() => {
        ...
        const allowHint = (performanceSummary.lowestCategory?.accuracy || 1) < 0.78 || difficulty <= 2;
