...
const FallingWordsGame = () => {
    ...
    useEffect(() => {
        ...
        try {
            ...
            const wordsToUse = (words && words.length > 0) ? words : (weighted && weighted.length ? weighted : getDueWords());

            if (!wordsToUse || wordsToUse.length === 0) {
                console.warn("No words available!");
                validWords.current = vocabulary || [];
            } else {
                validWords.current = wordsToUse;
            }
        } catch (e) {
    ...
    const gameLoop = (time) => {
        ...
        currentSpawnIntervalRef.current = Math.max(
            baseInterval * (band?.spawn || 1) * spawnTension / (1 + (combo * 0.1)) / difficultyMultiplier,
        ...
        activeWordsRef.current.forEach(word => {
            ...
            if (newY > 100) {
                mistakes++;
                if (!isZenModeRef.current) setLives(l => l > 0 ? l - 1 : 0);
