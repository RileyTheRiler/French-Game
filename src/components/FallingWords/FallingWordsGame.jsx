...
const FallingWordsGame = () => {
    ...
    const { getDueWords, updateWordProgress, markWordSeen, getPracticeQueue, vocabulary } = useVocabulary();
    const {
        stats,
    ...
    useEffect(() => {
        ...
        try {
            ...
            const weighted = getDueWords(); // simplified fallback
            const finalWords = (words && words.length > 0) ? words : (weighted && weighted.length > 0) ? weighted : vocabulary || [];
