...
const FallingWordsGame = () => {
    ...
    const { getDueWords, updateWordProgress, getWeightedPracticeWords, vocabulary, markWordSeen, CATEGORIES, getPracticeQueue } = useVocabulary();
    ...
    useEffect(() => {
        ...
        try {
            ...
            const finalWords = (weighted && weighted.length) ? weighted : (words.length ? words : getDueWords());
    ...
    const grantSessionRewards = useCallback(() => {
        ...
        const reward = calculateRewards('fallingWords', {
            ...
            maxCombo,
            wordsCaught,
            livesRemaining: 3, // Assuming max lives for now as we use Time Attack
    ...
    const handleInputChange = (e) => {
        ...
        if (matchIndex !== -1) {
            ...
            setScore(s => {
                const newScore = Math.floor(s + (10 * totalMultiplier));
                // Record event
    ...
    }, [renderedWords, performanceSummary, difficulty, CATEGORIES, bandRef]);
        ...
        <GameLayout
            ...
            headerRight={
                <div className="flex items-center gap-4">
                    ...
                    <Badge variant="primary" className="text-lg py-1 px-4" data-testid="score-badge">
                    ...
                    {!isZenMode && (
                        <div className="relative">
                            <Badge variant={timeLeft <= 10 ? "destructive" : "secondary"} className={`text-lg py-1 px-4 w-24 justify-center font-mono ${timeLeft <= 10 ? 'animate-pulse' : ''}`} data-testid="timer-badge">
                                <Clock size={16} className="mr-2 opacity-50" />
                                <span data-testid="timer-display">{Math.floor(timeLeft / 60)}:{(timeLeft % 60).toString().padStart(2, '0')}</span>
                            </Badge>
