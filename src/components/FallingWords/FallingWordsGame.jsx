...
const FallingWordsGame = () => {
    ...
    const { getDueWords, updateWordProgress, markWordSeen, getPracticeQueue, CATEGORIES, vocabulary } = useVocabulary();
    const {
        stats,
    ...
    const grantSessionRewards = useCallback(() => {
        ...
        const reward = calculateRewards('fallingWords', {
            ...
            maxCombo,
            wordsCaught,
            livesRemaining: 3, // Dummy value as we don't use lives
    ...
    const gameLoop = (time) => {
        ...
        activeWordsRef.current.forEach(word => {
            ...
            const categoryPressure = 1 + ((1 - (word.categoryAccuracy || 1)) * 0.3);
            const responseRelief = (word.categoryResponse || 2000) > 3500 ? 0.85 : 1;
    ...
    }, [getPracticeQueue, getDueWords, vocabulary]); // removed startGame from deps to avoid recursion if startGame isn't stable. But startGame relies on gameLoop etc. Ideally useEffect should just depend on init props.
                ...
                <div className="w-full h-full relative">
                    {renderedWords.map(word => (
                        <WordItem
                            ...
                            hintDelay={difficultySettings?.hintDelay ?? 8}
