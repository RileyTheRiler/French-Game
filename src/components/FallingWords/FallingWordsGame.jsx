...
const FallingWordsGame = () => {
    ...
    const { stats, recordCategoryPerformance, setModeDifficulty, addXP, addCoins, updateDailyStat, incrementStat, globalDifficulty, difficultySettings, offlineAudio } = useProgress();
    ...
    useEffect(() => {
        ...
        try {
            // Priority: Practice Queue -> Vocabulary -> Fallback
            const queue = getPracticeQueue ? getPracticeQueue('fallingWords', 40) : [];
            if (queue && queue.length > 0) {
                validWords.current = queue;
            } else {
                validWords.current = vocabulary || [];
            }

            if (validWords.current.length === 0) {
    ...
    const performanceSummary = useMemo(() => {
        ...
        entries.forEach(([category, perf]) => {
            ...
            if (!lowestCategory || accuracy < (lowestCategory?.accuracy || 1)) {
    ...
    const spawnWord = () => {
        ...
        const newWord = {
            ...
            target: randomWord,
            mastery: randomWord.level || 0,
            lastSeen: randomWord.lastSeen || 0,
    ...
    const gameLoop = (time) => {
        ...
        // Combine difficulty logic from both versions
        const baseSpeed = INITIAL_FALL_SPEED * (band?.speed || 1);
        const dynamicScale = 1 + difficultyProgress * (band?.speed || 1);

        const effectiveSpeed = baseSpeed * dynamicScale * flowMultiplier * performanceSpeedMod * responseSpeedMod * difficultyMultiplier;
        ...
        currentSpawnIntervalRef.current = Math.max(
            (baseInterval / difficultyMultiplier) * (band?.spawn || 1) * spawnTension / (1 + (combo * 0.1)),
        ...
        activeWordsRef.current.forEach(word => {
            ...
            if (newY > 100) {
                ...
                updateWordProgress(word.wordId, 'again'); // Use both for safety
    ...
    const grantSessionRewards = useCallback(() => {
        ...
        // Calculate reward based on score and mode
        // Note: 'lives' is not tracked in Zen mode or Time Attack (which uses time),
        // so we pass effectively infinite lives or just rely on score/combo
        ...
        updateDailyStat('dailyWordsCaught', wordsCaught, 'add');
    ...
    const handleInputChange = (e) => {
        ...
        if (matchIndex !== -1) {
            ...
            setScore(s => {
                const newScore = Math.floor(s + (basePoints * totalMultiplier));
                // Record event for Ghost
    ...
    const hintData = useMemo(() => {
        if (!renderedWords.length) return null;
        ...
        const allowHint = (performanceSummary.lowestCategory?.accuracy || 1) < 0.78 || difficultySetting <= 2 || (bandRef.current?.hintBias || 1) > 1;
    ...
    }, [renderedWords, performanceSummary, difficultySetting, CATEGORIES]);
    ...
    }, [gameOver, onExit]); // eslint-disable-line react-hooks/exhaustive-deps
