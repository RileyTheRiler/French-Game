...
// const FALL_SPEED_INCREMENT = 0.05; // Unused in HEAD loop?
...
const FallingWordsGame = () => {
    ...
    const { getPracticeQueue, updateWordProgress, markWordSeen, vocabulary, getDueWords, getWeightedPracticeWords } = useVocabulary();
    const {
        stats,
    ...
    // Shadowing State (from other branch)
    ...
    // Speech Recognition Setup (from Other branch)
    useEffect(() => {
    ...
    const spawnWord = () => {
        ...
        const newWord = {
            ...
            target: randomWord, // for shadowing
            gender: randomWord.gender
        };

        markWordSeen && markWordSeen(randomWord.id);
    ...
    const grantSessionRewards = useCallback(() => {
        ...
        const reward = calculateRewards('fallingWords', {
            ...
            maxCombo,
            wordsCaught,
            livesRemaining: 3, // Dummy value since we use time
    ...
    const gameLoop = (time) => {
        ...
        activeWordsRef.current.forEach(word => {
            ...
            if (newY > 100) {
                ...
                recordCategoryPerformance(word.category, { success: false, responseTime: performance.now() - word.spawnTime, mode: 'fallingWords' });
            } else {
                word.y = newY;
                nextWords.push(word);
            }
    ...
    const handleInputChange = (e) => {
        ...
        if (matchIndex !== -1) {
            ...
            recordCategoryPerformance(word.category, { success: true, responseTime: performance.now() - word.spawnTime, mode: 'fallingWords' });
    ...
    useEffect(() => {
        if (gameOver) grantSessionRewards();
