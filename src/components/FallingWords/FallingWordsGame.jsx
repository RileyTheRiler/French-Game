...
import DifficultyDial, { getDifficultyConfig } from '../ui/DifficultyDial';
...
const FallingWordsGame = () => {
    ...
    const { getDueWords, updateWordProgress, getWeightedPracticeWords, getPracticeQueue, markWordSeen, vocabulary, CATEGORIES } = useVocabulary();
    ...
    useEffect(() => {
        ...
        try {
            // Priority: Weighted Practice > Due Words > Practice Queue > Fallback Vocabulary
            let words = [];
            if (getWeightedPracticeWords) {
                words = getWeightedPracticeWords(40);
            }
            if ((!words || words.length === 0) && getDueWords) {
                words = getDueWords();
            }
            if ((!words || words.length === 0) && getPracticeQueue) {
    ...
    const grantSessionRewards = useCallback(() => {
        ...
        // Mock lives logic for now since we removed the lives state in merge
        const lives = 3;
    ...
    const gameLoop = (time) => {
        ...
        currentFallSpeedRef.current = effectiveSpeed;
