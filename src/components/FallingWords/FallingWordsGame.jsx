...
import { Mic, Volume2, TrendingUp, Clock, Ghost, Swords } from 'lucide-react';
...
const FallingWordsGame = () => {
    ...
    useEffect(() => {
        ...
        try {
            // Priority: Practice Queue -> Weighted -> Due -> All Vocabulary
            let words = [];
            if (getPracticeQueue) words = getPracticeQueue('fallingWords', 40);

            if (!words || words.length === 0) {
                words = getWeightedPracticeWords ? getWeightedPracticeWords(40) : getDueWords();
            }
    ...
    useEffect(() => {
        ...
        if (setModeDifficulty) setModeDifficulty('fallingWords', difficulty);
    ...
    const grantSessionRewards = useCallback(() => {
        ...
        const reward = calculateRewards ? calculateRewards('fallingWords', {
            score,
            maxCombo,
            wordsCaught,
            livesRemaining: 3, // Lives concept removed but kept for API
            zenMode: isZenModeRef.current
        }) : { xp: Math.floor(score / 10), coins: Math.floor(score / 50) };

        setSessionReward(reward);
        if (addXP) addXP(reward.xp);
        if (addCoins) addCoins(reward.coins);
        if (updateDailyStat) updateDailyStat('dailyStreak', maxCombo, 'max');
        if (incrementStat) incrementStat('gamesPlayed', 1);
    ...
    const gameLoop = (time) => {
        ...
        currentFallSpeedRef.current = Math.min(
            baseSpeed * dynamicScale * flowMultiplier * performanceSpeedMod * responseSpeedMod * difficultyMultiplier,
            MAX_FALL_SPEED * 1.5
        );
        ...
        activeWordsRef.current.forEach(word => {
            ...
            if (newY > 100) {
                mistakes++;
                updateWordProgress(word.wordId, 'again'); // Use string grade
    ...
    const handleInputChange = (e) => {
        ...
        if (matchIndex !== -1) {
            ...
            updateWordProgress(word.wordId, 'good'); // String grade
            setWordsCaught(prev => prev + 1);
            if (updateDailyStat) updateDailyStat('dailyWordsCaught', 1);
            if (logWordAttempt) logWordAttempt(word.category || 'General', true, performance.now() - word.spawnTime);
            ...
            setScore(s => {
                const newScore = Math.floor(s + (12 * comboMultiplier * bandScore * accuracyBoost * speedBoost));
                // Record event
            ...
            setCombo(c => {
                ...
                if (updateDailyStat) updateDailyStat('dailyStreak', newCombo, 'max');
                return newCombo;
            });
