...
const WordItem = memo(({ text, x, y, isMatched, hint, spawnTime, hintDelay = 8, mastery, lastSeen }) => {
    ...
    useEffect(() => {
        ...
        // Use performance.now() if spawnTime is high-res, else Date.now()
        // Ideally should match source. Assuming FallingWords passes performance.now()
        const now = performance.now();
        // If spawnTime is clearly timestamp (huge number), use Date.now()
        const isTimestamp = spawnTime > 1000000000000;
        const current = isTimestamp ? Date.now() : now;

        const elapsed = current - spawnTime;
