...
const StudySession = () => {
    ...
    useEffect(() => {
        // Initial load of due words, possibly filtered
        const baseDue = getDueWords(); // or getPracticeQueue('study', 12) if preferred, but filtering implies full access
        ...
        // Limit to a reasonable session size if filtering returns too many
        const sessionWords = filtered.slice(0, 20);

        setDueWords(sessionWords);
        setCurrentIndex(0);
        setIsFlipped(false);
        // If no words, don't auto-complete session, let EmptyState handle it
        ...
        if (sessionWords.length > 0) {
            preloadAudioForWords(sessionWords);
        }
    }, [getDueWords, filterCEFR, filterCategory, preloadAudioForWords]);
    ...
    const handleResult = (grade) => {
        ...
        const success = passing; // alias for logic
