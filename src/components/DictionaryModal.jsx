...
const VocabItem = memo(({ word, toggleSaveWord, togglePinWord, snoozeWord, clearSnooze, playWordAudio, offlineAudio, now }) => {
...
const DictionaryModal = ({ onClose, initialSearchTerm = '' }) => {
    ...
    // Optimize: Memoize filtered lists to prevent re-calculation on every render
    ...
    // Optimize: Decouple time updates from data updates.
    // 'now' only updates every minute, preventing list re-renders when vocabulary changes (e.g., toggling save).
