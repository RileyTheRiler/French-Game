...
const ConversationSimulator = () => {
    ...
    const { addXP, addCoins, globalDifficulty, difficultySettings, updateDailyStat, incrementStat } = useProgress();
    ...
    const grantRewards = (success) => {
        ...
        if (success) {
            ...
            if (updateDailyStat) updateDailyStat('dailyConversations', 1);
            if (incrementStat) incrementStat('conversationsCompleted', 1);
        }
    };
    ...
    const handleTypedSubmit = () => {
        ...
        if (matchResult && matchResult.score > 0.4) {
            ...
            setStepsTaken(prev => prev + 1);
            // Off-script isn't necessarily a mistake, but if not understood, maybe?
            // Let's not count as mistake unless it's explicitly bad.
