...
const ConversationSimulator = () => {
    ...
    const grantRewards = (success) => {
        ...
        // Mock stats for calculation if not tracking granularly yet
        const mistakes = 0;
        const stepsTaken = history.length;

        const reward = calculateRewards ? calculateRewards('conversation', {
            baseXp: activeScenario.xpReward,
            difficulty: activeScenario.difficulty || 'intermediate',
            mistakes,
            steps: stepsTaken,
            success
        }) : { xp: activeScenario.xpReward, coins: 10 };
