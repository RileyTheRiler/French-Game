...
export const SocialProvider = ({ children }) => {
    ...
    // Derived state for current progress
    const activeChallenge = React.useMemo(() => {
        const userContribution = Math.max(0, stats.xp - userCoopStartXp);
        const total = Math.min(activeChallengeBase.target, userContribution + friendsProgress);
        return {
            ...activeChallengeBase,
            current: total,
            isCompleted: total >= activeChallengeBase.target
        };
    }, [activeChallengeBase, stats.xp, userCoopStartXp, friendsProgress]);
