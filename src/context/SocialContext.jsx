...
export const SocialProvider = ({ children }) => {
    ...
    // Store only the static configuration of the challenge
    const [activeChallengeBase, setActiveChallengeBase] = useState(() => ({
        id: 'chal_weekly_xp',
    ...
    // Derive the current progress during render
    const userContribution = Math.max(0, stats.xp - userCoopStartXp);
    const totalProgress = Math.min(activeChallengeBase.target, userContribution + friendsProgress);
    const isChallengeCompleted = totalProgress >= activeChallengeBase.target;
