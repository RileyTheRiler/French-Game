...
export const ProgressProvider = ({ children }) => {
        ...
        if (saved) {
            try {
                ...
                return {
                    ...
                    learningProfile: { ...defaultStats.learningProfile, ...(parsed.learningProfile || {}) },
                    focusModeStats: { ...defaultStats.focusModeStats, ...(parsed.focusModeStats || {}) },
    ...
    }, [ensureSeasonWindow, stats.doubleXpUntil, updateDailyStat]);
    ...
    useEffect(() => {
        checkAchievements();
    }, [stats.xp, stats.wordsLearned, stats.streak, checkAchievements]);
    ...
    const logDreamGoal = useCallback((goalKey) => {
        setStats(prev => ({
            ...prev,
            dreamGoals: {
                ...prev.dreamGoals,
    ...
    const updateMemoryPalaceRoom = useCallback((roomKey, updates) => {
        setStats(prev => ({
            ...prev,
            memoryPalace: {
                ...prev.memoryPalace,
    ...
    const contextValue = useMemo(() => ({
        ...
        audioEnabled, offlineAudio, reducedMotion, colorTheme,
        addCoins, spendCoins, buyItem, consumeItem, incrementStat, checkAchievements,
        activateDoubleXP, isDoubleXpActive, updateDailyStat, hydrateProgress,
        completeOnboarding, applyPlacementResult, setTargetCefr, setWeeklyGoal,
        setModeDifficulty, recordCategoryPerformance, logWordAttempt, updateUserGoals,
        updateDifficultySettings, getWeeklySummary, markWeeklyRecapSeen, markWordStrength,
        addToReviewQueue, removeFromReviewQueue, updateDailyXPGoalFn, trackConversationSession,
        setLearningProfile, setGlobalDifficulty, updateWeeklyGoal, recordWeeklySession,
        isWeeklyGoalMet, recordFocusModeCompletion, logConceptAttempt, getWeakConceptsList,
        updateStats, updateRegionProgress, updateMediaProgress, updateSurvivalBest,
        updateCognitiveState, logDreamGoal, updateMemoryPalaceRoom
    ]);
