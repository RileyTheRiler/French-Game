...
export const VocabularyProvider = ({ children }) => {
    ...
    const getDueWords = useCallback(() => {
        const now = Date.now();
        // Optimization: Schwartzian transform precomputes priority to avoid O(N log N) expensive score calculations
    ...
    const getWeightedPracticeWords = useCallback((limit = 30) => {
        // Optimization: Schwartzian transform precomputes priority and maps object references directly instead of using spread (...) for memory efficiency
        return vocabulary
            .map(word => ({
                word,
