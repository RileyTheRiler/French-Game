...
const hydrateWord = (word) => {
    ...
    // Combine logic from enhanceWord and hydrateWord
    return {
        ...word,
        // Audio/Phonetics
        ...
        // SRS & Progress
        srs,
        level: word.level ?? Math.max(1, srs.repetition || 1),
        nextReview: word.nextReview ?? srs.dueDate ?? 0,
...
export const VocabularyProvider = ({ children }) => {
    ...
    const togglePinWord = useCallback((wordId) => {
        setVocabulary(prev => prev.map(word =>
            word.id === wordId ? { ...word, pinned: !word.pinned, snoozeUntil: !word.pinned ? null : word.snoozeUntil } : word
        ));
    }, []);
    ...
    const snoozeWord = useCallback((wordId, durationMs = 6 * 60 * 60 * 1000) => {
        ...
        setVocabulary(prev => prev.map(word =>
            word.id === wordId ? { ...word, snoozeUntil: until, pinned: false } : word
        ));
    }, []);

    const clearSnooze = useCallback((wordId) => {
        setVocabulary(prev => prev.map(word =>
            word.id === wordId ? { ...word, snoozeUntil: null } : word
        ));
    }, []);
    ...
    const markWordSeen = useCallback((wordId) => {
        const now = Date.now();
        setVocabulary(prev => prev.map(word =>
            word.id === wordId ? { ...word, lastSeen: now } : word
        ));
    }, []);
    ...
    const createDeck = useCallback((name, description = '', wordIds = [], color = 'indigo') => {
        const newDeck = {
            id: `deck_${Date.now()}`,
            name, description, wordIds, color, createdAt: Date.now(), updatedAt: Date.now()
        };
    ...
    const addWordToDeck = useCallback((deckId, wordId) => {
        setCustomDecks(prev => prev.map(deck => {
            ...
            return { ...deck, wordIds: [...deck.wordIds, wordId], updatedAt: Date.now() };
        }));
    }, []);
    ...
    const removeWordFromDeck = useCallback((deckId, wordId) => {
        setCustomDecks(prev => prev.map(deck => {
            if (deck.id !== deckId) return deck;
            return { ...deck, wordIds: deck.wordIds.filter(id => id !== wordId), updatedAt: Date.now() };
        }));
    }, []);
    ...
    const exportDeck = useCallback((deckId) => {
        ...
        // Browser specific code for downloading - omitting safe execution check as this is client side
        return exportData;
    }, [customDecks, vocabulary]);
    ...
    const contextValue = useMemo(() => ({
        ...
        vocabulary, updateWordProgress, markWordSeen, togglePinWord, snoozeWord, clearSnooze, toggleSaveWord,
        getDueWords, getPracticeQueue, getWeightedPracticeWords, resetVocabulary,
        downloadAudioOnce, hydrateVocabulary, preloadAudioForWords, playWordAudio,
        addCustomWord, customDecks, createDeck, updateDeck, deleteDeck, getDeckWords, addWordToDeck, removeWordFromDeck,
