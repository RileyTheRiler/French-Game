...
const ConjugationBlitz = () => {
    ...
    // FIX: To properly access score without re-creating endGame, use a Ref for score.
    const scoreRef = useRef(0);
    useEffect(() => { scoreRef.current = score; }, [score]);

    // Redefine endGame to use ref
    const endGameWithScore = useCallback(() => {
        clearInterval(timerRef.current);
        setStatus('finished');
        SoundManager.playLevelUp();
        addXP(scoreRef.current * 2);
    }, [addXP]);
