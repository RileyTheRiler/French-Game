...
export const sortByReviewPriority = (words) => {
    ...
    // Schwartzian transform: pre-calculate priorities to avoid re-computing
    // expensive retention probabilities during sort comparison.
    // Complexity reduces from O(N log N * C) to O(N * C + N log N)
    const withPriority = words.map(word => {
    ...
    return withPriority.map(item => item.word);
};
