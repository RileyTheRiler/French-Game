...
export async function verifyPassword(stored, password) {
    ...
    const originalHashBytes = new Uint8Array(originalHashHex.match(/.{1,2}/g).map(byte => parseInt(byte, 16)));

    // Constant-time comparison
    if (derivedHashBytes.length !== originalHashBytes.length) {
        return false;
    }

    let result = 0;
    for (let i = 0; i < derivedHashBytes.length; i++) {
        result |= derivedHashBytes[i] ^ originalHashBytes[i];
    }
