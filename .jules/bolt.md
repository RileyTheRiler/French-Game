...
## 2024-05-24 - Context Memoization across multiple contexts
**Learning:** Found multiple contexts (`AuthContext`, `SyncContext`, `A11yContext`, `CommunityContext`, `SocialContext`, `MessagingContext`) lacking memoization for their values, which causes unnecessary re-renders in all consumers whenever the contexts update.
**Action:** Implemented `useMemo` for the context values with detailed performance impact comments.
