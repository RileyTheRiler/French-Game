...
## 2026-04-04 - Context Value Memoization
**Learning:** Found that `CommunityContext` was recreating its `value` object on every render because it lacked `useMemo`. This causes unnecessary re-renders in all consumer components whenever context state (like `communityStats`) updates.
**Action:** Always wrap context provider `value` objects in `useMemo` and verify the dependency array includes all dynamic properties and methods to prevent performance regressions.
