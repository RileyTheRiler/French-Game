...
## 2026-04-01 - Memoization in Context Providers
**Learning:** React Context Providers that pass unmemoized objects (e.g., `const value = { ... }`) cause all consumers to unnecessarily re-render whenever the Provider's parent component re-renders.
**Action:** Always wrap the `value` object in Context Providers with `useMemo` and ensure that any functions included in the dependencies (like those wrapped in `useCallback`) are declared before they are referenced to avoid initialization errors.
