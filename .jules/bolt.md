...
## 2024-05-24 - Context Memoization
**Learning:** Context Providers must wrap their values in memoization hooks. Otherwise, all consumers re-render every time the provider re-renders, causing significant performance overhead in large React applications.
**Action:** Always memoize the value object passed to a context provider component, ensuring to explicitly import the appropriate memoization hook to prevent reference errors.
