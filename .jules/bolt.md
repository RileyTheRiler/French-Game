...
## 2024-05-24 - Missing Context Value Memoization
**Learning:** Found an unmemoized context value object in `SyncContext.jsx` (using `<SyncContext.Provider value={{...}}>`). In React, this creates a new object reference on every render of the Provider, forcing all consuming components to unnecessarily re-render even if the actual state hasn't changed.
**Action:** Always memoize context value objects using `useMemo` and ensure all dynamic local dependencies (like state variables and callback functions) are correctly listed in the dependency array.
