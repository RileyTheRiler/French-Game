...
## 2024-05-24 - Missing Memoization in CommunityContext
**Learning:** Found another Context Provider (`CommunityContext`) without memoization on its `value` object. Like `ToastContext`, this causes unnecessary re-renders in all consuming components whenever the provider re-renders (due to local state or dependency changes).
**Action:** Continuously audit Context Providers to ensure `useMemo` is applied to their `value` props to prevent widespread performance degradation.
