...
## 2024-09-10 - Unmemoized Context Values
**Learning:** In a heavily componentized app, providing unmemoized objects as Context values causes the Context Provider to create a new object identity on every single render. This triggers unnecessary re-renders in all downstream consumers.
**Action:** Always wrap Context value objects with `useMemo`, ensuring they only change when their dependency array values actually update.
