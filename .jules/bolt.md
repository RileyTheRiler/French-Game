...
## 2024-05-23 - Array Sorting Overhead with Object Spreads
**Learning:** During array sorting operations using Schwartzian transforms (Decorate-Sort-Undecorate), using the object spread operator (`...word`) in the mapping phase to attach temporary sorting weights causes significant performance degradation due to shallow copying potentially massive arrays of large objects.
**Action:** When decorating items for sorting, map to an object containing a direct reference to the original item (e.g., `{ word, score }`) rather than cloning it. Extract the original item in the undecorate phase.
