...
## 2024-05-24 - Unmemoized Context Values
**Learning:** `AuthContext` (and several other context providers) was passing a newly created object to `value` on every render. This causes all consuming components to unnecessarily re-render whenever the provider re-renders, even if the actual context data hasn't changed.
**Action:** Always wrap context `value` objects in `useMemo`, with dependencies explicitly listed, to prevent widespread unnecessary re-renders across the application.
