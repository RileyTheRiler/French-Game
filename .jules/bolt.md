...
## 2024-05-24 - Unnecessary Re-renders in List Components
**Learning:** In interactive components like `SentenceBuilder` where items are moved between lists, passing unstable callback functions (`onClick`) to children causes the entire list to re-render on every interaction.
**Action:** Use `React.memo` for list items (like `WordTile`) and `useCallback` for event handlers. This reduces re-renders from O(N) to O(1) for drag/drop or click interactions. Also, be wary of merge conflicts (`<<<<<<<`) hiding in files, as they break the build and prevent verification of optimizations.
