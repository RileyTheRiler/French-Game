...
## 2025-02-20 - Memoizing React Context Providers
**Learning:** React Context values must be memoized using the memoization hook to prevent consumers from re-rendering unnecessarily on every state update or provider re-render.
**Action:** Always wrap the value object passed to Context Providers in a memoization hook with all required dependencies explicitly listed.
