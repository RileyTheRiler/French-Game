...
## 2024-05-25 - Custom Interactive Visual Controls Accessibility
**Learning:** Custom interactive visual controls (like color swatches and size selectors) lack native semantic meaning. When implementing these, screen readers announce them generically or not at all if only visual cues are used.
**Action:** Explicitly add appropriate labels to the interactive container elements and apply hidden attributes to their internal decorative elements to ensure screen reader accessibility without redundant announcements.
