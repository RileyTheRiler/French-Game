...
## 2024-05-25 - Semantic Sliders
**Learning:** Custom slider components (like `DifficultyDial`) often lack context for screen readers. Using `aria-valuetext` bridges the gap between a numeric value (e.g., 25) and its semantic meaning (e.g., 'Beginner').
**Action:** Always map slider values to human-readable strings via `aria-valuetext` when the number alone is insufficient.
