...
## 2025-05-08 - Accessible Pagination Dots
**Learning:** Pagination controls (like dots) often lack accessible grouping and state. Users navigating with screen readers or keyboards need `role="group"`, `aria-label` per dot, `aria-current`, and visible focus indicators.
**Action:** Always wrap pagination dots in an labeled group, add individual ARIA labels indicating their target, and implement explicit `focus-visible` styles.
