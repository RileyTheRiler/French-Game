...
## 2024-06-06 - Missing accessible labels on icon-only buttons
**Learning:** Custom button components with only icons lack accessible labels by default, making them inaccessible to screen readers.
**Action:** Always provide accessible labels indicating the purpose of icon-only buttons to ensure they are properly read by screen readers.
