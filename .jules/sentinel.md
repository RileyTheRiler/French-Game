...
## 2025-02-14 - Unsafe DOM Manipulation in Debug Script
**Vulnerability:** `innerHTML +=` was used on `document.body` in `src/main_debug_dynamic.jsx` to render error messages containing user/system-provided input (`path` and `e.message`), making it susceptible to XSS.
**Learning:** Even in non-production or debug scripts, direct HTML assignment can be exploited or cause unintended side effects (like destroying event listeners and React roots due to DOM re-parsing).
**Prevention:** Use safe DOM manipulation like `document.createElement('h3')` and `element.textContent = ...` followed by `document.body.appendChild(element)` instead of `innerHTML`.
