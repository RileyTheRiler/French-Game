...
## 2025-04-07 - Insecure JSON parsing and information leakage
**Vulnerability:** Uncaught exceptions and stack trace leakage from malformed JSON in `importData` and `importDeck`.
**Learning:** Directly parsing external input with `JSON.parse` without a try-catch and type validation can cause unhandled TypeErrors and leak error details.
**Prevention:** Always wrap `JSON.parse()` in a try-catch block, check that the result is a non-null object, and catch and replace errors with generic messages to prevent information leakage.
