...
## 2025-02-14 - Unsafe JSON Parsing in File Import
**Vulnerability:** External file imports parsed JSON without error handling and lacked validation that the result was an object before accessing properties, leading to unhandled exceptions and potential type errors.
**Learning:** Directly parsing files uploaded by users can crash the app if the contents are malformed or a malicious payload is structured unexpectedly.
**Prevention:** Always wrap `JSON.parse` of external inputs in an error handling block and explicitly verify the type of the parsed result before proceeding.
