...
## 2025-02-14 - Unsafe JSON Parsing
**Vulnerability:** User-provided files were parsed with `JSON.parse()` without a `try...catch` block, and without checking if the result was a valid object.
**Learning:** Malformed JSON could crash the application, and accessing properties of a null or non-object result causes unhandled TypeErrors. Exposing parsing errors could leak information.
**Prevention:** Always wrap `JSON.parse()` of external inputs in `try...catch`. Catch `SyntaxError` to throw safe, generic messages. Verify the parsed result is a non-null object before use.
