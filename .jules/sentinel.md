...
## 2025-02-14 - Insecure JSON parsing in external imports
**Vulnerability:** External JSON files were parsed using JSON.parse without try...catch, leading to uncaught exceptions if the file was malformed.
**Learning:** When parsing user-provided files or external inputs, unhandled exceptions can crash the application or leak information via stack traces.
**Prevention:** Always wrap JSON.parse in a try...catch block. Catch SyntaxError and throw safe, generic error messages. Verify the parsed result is a non-null object before accessing properties.
