...
## 2024-03-24 - Fix Insecure Deserialization in SyncContext
**Vulnerability:** Unsafe JSON parsing in SyncContext.jsx importData allowed malformed JSON files to crash the application by throwing unhandled syntax errors.
**Learning:** `JSON.parse` must always be wrapped in a try/catch block when dealing with user-uploaded files, and exceptions should be masked behind generic error messages to prevent potential information leakage.
**Prevention:** In functions reading user-provided files (such as `importData` or `importDeck`), wrap file reading and parsing logic in try/catch and emit safe error states.
