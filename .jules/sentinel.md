...
## 2025-02-15 - Unhandled JSON parsing in file imports
**Vulnerability:** File import logic used `JSON.parse()` without error handling, which could lead to app crashes when given malformed files, or `TypeError`s if a non-object payload is uploaded.
**Learning:** Functions that accept user-provided file contents for hydration must be treated as untrusted boundaries. Developers often assume the exported format will be exactly matched by the imported format, missing edge cases where users manipulate or upload arbitrary files.
**Prevention:** Always wrap `JSON.parse()` in a `try...catch` block when processing external data. Additionally, verify that the parsed result is the expected structural type (e.g., a non-null object) before accessing properties to prevent cascading failures.
