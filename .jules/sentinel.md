...
## 2024-05-25 - Insecure JSON Parsing and Error Leakage
**Vulnerability:** User-provided JSON files were parsed without verifying if the result was a non-null object, and parsing errors leaked raw exception messages.
**Learning:** Directly parsing external data and using it without type checking can lead to application crashes (TypeErrors), while exposing raw error messages can leak implementation details.
**Prevention:** Always wrap JSON parsing of external data in an error-handling block, verify the parsed result is a non-null object before use, and throw generic error messages to avoid leaking sensitive information.
