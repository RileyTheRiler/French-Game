...
## 2026-03-19 - Prevent unhandled exceptions during JSON file parsing
**Vulnerability:** `JSON.parse()` was called on user-uploaded file content without a `try...catch` block, which could cause uncaught `SyntaxError` exceptions and application crashes if malformed JSON is provided.
**Learning:** This existed because file imports were assumed to be well-formed, bypassing standard input validation.
**Prevention:** Always wrap `JSON.parse()` calls that handle external or user-provided data in a `try...catch` block and throw safe, generic error messages (e.g., 'Invalid file format') to prevent information leakage.
